...
## Fuerzo Limpieza completa de caché para composer y laravel
php artisan clear-compiled >> $LOG 2>> $LOG
php artisan cache:clear >> $LOG 2>> $LOG
php artisan config:clear >> $LOG 2>> $LOG
php artisan package:discover >> $LOG 2>> $LOG
php artisan queue:flush >> $LOG 2>> $LOG
php artisan route:clear >> $LOG 2>> $LOG
php artisan view:clear >> $LOG 2>> $LOG
php artisan optimize:clear >> $LOG 2>> $LOG
composer dump-autoload >> $LOG 2>> $LOG
